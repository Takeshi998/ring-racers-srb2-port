...
Gles2Rhi::Gles2Rhi(std::unique_ptr<Gles2Platform>&& platform, Gles2LoadFunc load_func) : platform_(std::move(platform))
{
	gl_ = std::make_unique<GladGLES2Context>();
	gladLoadGLES2Context(gl_.get(), reinterpret_cast<GLADloadfunc>(load_func));
}

Gles2Rhi::~Gles2Rhi() = default;

rhi::Handle<rhi::Texture> Gles2Rhi::create_texture(const rhi::TextureDesc& desc)
{
...
void Gles2Rhi::destroy_texture(rhi::Handle<rhi::Texture> handle)
{
	...
	Gles2Texture casted = texture_slab_.remove(handle);
...
void Gles2Rhi::update_texture_settings(
	Handle<Texture> texture,
...
rhi::Handle<rhi::Buffer> Gles2Rhi::create_buffer(const rhi::BufferDesc& desc)
{
...
void Gles2Rhi::destroy_buffer(rhi::Handle<rhi::Buffer> handle)
{
	...
	Gles2Buffer casted = buffer_slab_.remove(handle);
...
rhi::Handle<rhi::Renderbuffer> Gles2Rhi::create_renderbuffer(const rhi::RenderbufferDesc& desc)
{
	...
	gl_->RenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH_COMPONENT16, desc.width, desc.height);
	GL_ASSERT;

	Gles2Renderbuffer rb;
...
void Gles2Rhi::destroy_renderbuffer(rhi::Handle<rhi::Renderbuffer> handle)
{
	...
	Gles2Renderbuffer casted = renderbuffer_slab_.remove(handle);
	GLuint name = casted.renderbuffer;
	gl_->DeleteRenderbuffers(1, &name);
	GL_ASSERT;
}

rhi::Handle<rhi::Program> Gles2Rhi::create_program(const ProgramDesc& desc)
{
...
void Gles2Rhi::destroy_program(Handle<Program> handle)
{
	SRB2_ASSERT(program_slab_.is_valid(handle) == true);
	Gles2Program casted = program_slab_.remove(handle);
...
void Gles2Rhi::apply_default_framebuffer(bool clear)
{
...
void Gles2Rhi::apply_framebuffer(const RenderPassBeginInfo& info, bool allow_clear)
{
	auto fb_itr = framebuffers_.find(Gles2FramebufferKey {info.color_attachment, info.depth_stencil_attachment});
...
void Gles2Rhi::push_default_render_pass(bool clear)
{
	SRB2_ASSERT(platform_ != nullptr);
...
void Gles2Rhi::push_render_pass(const RenderPassBeginInfo& info)
{
	render_pass_stack_.push_back(info);
	apply_framebuffer(info, false);
}

void Gles2Rhi::pop_render_pass()
{
...
void Gles2Rhi::bind_program(Handle<Program> program)
{
	SRB2_ASSERT(render_pass_stack_.empty() == false);
	Gles2Program& prog = program_slab_[program];
...
void Gles2Rhi::bind_vertex_attrib(const char* name, Handle<Buffer> buffer, VertexAttributeFormat format, uint32_t offset, uint32_t stride)
{
	...
	Gles2Program& prog = program_slab_[*current_program_];
	Gles2Buffer& cast_buffer = buffer_slab_[buffer];
...
void Gles2Rhi::bind_index_buffer(Handle<Buffer> buffer)
{
	SRB2_ASSERT(current_program_.has_value());
...
void Gles2Rhi::set_uniform(const char* name, float value)
{
	SRB2_ASSERT(current_program_.has_value());
	Gles2Program& cast = program_slab_[*current_program_];
...
void Gles2Rhi::set_uniform(const char* name, int value)
{
	SRB2_ASSERT(current_program_.has_value());
	Gles2Program& cast = program_slab_[*current_program_];
	GLint location = gl_->GetUniformLocation(cast.program, name);
...
void Gles2Rhi::set_uniform(const char* name, glm::vec2 value)
{
	SRB2_ASSERT(current_program_.has_value());
	Gles2Program& cast = program_slab_[*current_program_];
	GLint location = gl_->GetUniformLocation(cast.program, name);
...
void Gles2Rhi::set_uniform(const char* name, glm::vec3 value)
{
	SRB2_ASSERT(current_program_.has_value());
	Gles2Program& cast = program_slab_[*current_program_];
	GLint location = gl_->GetUniformLocation(cast.program, name);
...
void Gles2Rhi::set_uniform(const char* name, glm::vec4 value)
{
	SRB2_ASSERT(current_program_.has_value());
	Gles2Program& cast = program_slab_[*current_program_];
	GLint location = gl_->GetUniformLocation(cast.program, name);
...
void Gles2Rhi::set_uniform(const char* name, glm::ivec2 value)
{
	SRB2_ASSERT(current_program_.has_value());
	Gles2Program& cast = program_slab_[*current_program_];
	GLint location = gl_->GetUniformLocation(cast.program, name);
...
void Gles2Rhi::set_uniform(const char* name, glm::ivec3 value)
{
	SRB2_ASSERT(current_program_.has_value());
	Gles2Program& cast = program_slab_[*current_program_];
	GLint location = gl_->GetUniformLocation(cast.program, name);
...
void Gles2Rhi::set_uniform(const char* name, glm::ivec4 value)
{
	SRB2_ASSERT(current_program_.has_value());
	Gles2Program& cast = program_slab_[*current_program_];
	GLint location = gl_->GetUniformLocation(cast.program, name);
...
void Gles2Rhi::set_uniform(const char* name, glm::mat2 value)
{
	SRB2_ASSERT(current_program_.has_value());
	Gles2Program& cast = program_slab_[*current_program_];
	GLint location = gl_->GetUniformLocation(cast.program, name);
...
void Gles2Rhi::set_uniform(const char* name, glm::mat3 value)
{
	SRB2_ASSERT(current_program_.has_value());
	Gles2Program& cast = program_slab_[*current_program_];
	GLint location = gl_->GetUniformLocation(cast.program, name);
...
void Gles2Rhi::set_uniform(const char* name, glm::mat4 value)
{
	SRB2_ASSERT(current_program_.has_value());
	Gles2Program& cast = program_slab_[*current_program_];
	GLint location = gl_->GetUniformLocation(cast.program, name);
...
void Gles2Rhi::set_sampler(const char* name, uint32_t slot, Handle<Texture> texture)
{
	...
	Gles2Texture& tex = texture_slab_[texture];
...
void Gles2Rhi::set_rasterizer_state(const RasterizerStateDesc& desc)
{
...
void Gles2Rhi::set_viewport(const Rect& rect)
{
	SRB2_ASSERT(render_pass_stack_.empty() == false);
...
void Gles2Rhi::draw(uint32_t vertex_count, uint32_t first_vertex)
{
	SRB2_ASSERT(render_pass_stack_.empty() == false);
...
void Gles2Rhi::draw_indexed(uint32_t index_count, uint32_t first_index)
{
...
void Gles2Rhi::read_pixels(const Rect& rect, PixelFormat format, tcb::span<std::byte> out)
{
	SRB2_ASSERT(render_pass_stack_.empty() == false);
...
void Gles2Rhi::set_stencil_reference(CullMode face, uint8_t reference)
{
...
void Gles2Rhi::set_stencil_compare_mask(CullMode face, uint8_t compare_mask)
{
	SRB2_ASSERT(face != CullMode::kNone);
	SRB2_ASSERT(render_pass_stack_.empty() == false);
...
void Gles2Rhi::set_stencil_write_mask(CullMode face, uint8_t write_mask)
{
	SRB2_ASSERT(face != CullMode::kNone);
	SRB2_ASSERT(render_pass_stack_.empty() == false);
...
TextureDetails Gles2Rhi::get_texture_details(Handle<Texture> texture)
{
	SRB2_ASSERT(texture_slab_.is_valid(texture));
	auto& t = texture_slab_[texture];
...
Rect Gles2Rhi::get_renderbuffer_size(Handle<Renderbuffer> renderbuffer)
{
...
uint32_t Gles2Rhi::get_buffer_size(Handle<Buffer> buffer)
{
	SRB2_ASSERT(buffer_slab_.is_valid(buffer));
