...
struct Gles2Program : public rhi::Program
{
...
class Gles2Rhi final : public Rhi
{
	...
	Slab<Gles2Texture> texture_slab_;
	Slab<Gles2Buffer> buffer_slab_;
	Slab<Gles2Renderbuffer> renderbuffer_slab_;
	Slab<Gles2Program> program_slab_;

	Handle<Buffer> current_index_buffer_;

	srb2::HashMap<Gles2FramebufferKey, uint32_t> framebuffers_ {16};
...
public:
	Gles2Rhi(std::unique_ptr<Gles2Platform>&& platform, Gles2LoadFunc load_func);
	virtual ~Gles2Rhi();
